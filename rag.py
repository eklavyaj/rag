...
def populate_database_portfolio(conn, excel_file, sheet='low risk'):
...
def populate_database_history(conn, csv_folder):
    """"
    Populates the database with historical data.
    """
    
    # find all csv files in the csv folder
    all_csv_files = glob.glob(os.path.join(csv_folder,'**/*.csv'), recursive=True)
    ...
    for file in all_csv_files:
        ...
        df.to_sql("history_" + ticker.replace("-", "_"), con=conn, index=False, if_exists='replace')
...
def get_database(db, 
    ...
    populate_database_history(conn, csv_folder=csv_folder)
    populate_database_portfolio(conn,  excel_file=excel_file, sheet=portfolio)
    populate_database_csv(conn, csv_path=csv_path)
    
    sql_database = SQLDatabase(engine)
    
    # print("SQL Database object Initialized")
...
def get_query_engine(sql_database, service_context):
    ...
    # print("Query Engine with Table Retrieval Initialized")
    return query_engine
